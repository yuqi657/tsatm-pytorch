...
TSN Configurations:
    ...
    dropout_ratio:      {}
        ...
        planes = 512
        self.conv1 = nn.Conv2d(extract_feature, planes, kernel_size=1, bias=False)
    ...
    def forward(self, input):
        ...
        base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))    # shape here is (bs*n_seg, sample_len, h, w)
        # had changed base net, input size here is (batch_size*N_seg, fc_input, 7, 7)

        # # cascade
        # base_out = self.conv1(base_out)    # reduce channel
        # base_out = self.bn1(base_out)
        # base_out = self.relu(base_out)

        # base_out = base_out.view((-1, self.num_segments) + base_out.size()[1:]).transpose(1,2)
        # base_out = self.conv3d_spatial(base_out)
        # base_out = self.conv3d_temporal(base_out)
        # base_out = self.bn3d(base_out)
        # base_out = self.relu(base_out)

        # base_out = self.avgpool(base_out)
        # base_out = torch.flatten(base_out, 1)
        # base_out = self.drop(base_out)
        ...
        # parrallel
        # add time module
        at_out = self.conv1(self.base_model.frame_feature)    # reduce channel
        at_out = self.bn1(at_out)
        ...
        return 0.9 * output.squeeze(1) + 0.1 * at_out

        # TODO: extract features of the last conv
        # extract here
        # frame_feature = self.base_model.frame_feature
        # frame_feature = frame_feature.view((-1, self.num_segments) + frame_feature.size()[1:])
        # Shape of self.frame_feature here is [batch_size, N_seg, fc_input, 7, 7]
        # fc_input is 2048
        # print('frame_feature shape: ', frame_feature.shape)
        # print('self frame_feature shape: ', self.frame_feature.shape)
        # print('resnet fc layer: ', base_out.shape)
        # Shape of base out here is [batch_size*N_seg, fc_input]


        # base out size here is ( 2048, 7, 7)
        # if self.dropout > 0:
        #     base_out = self.new_fc(base_out)
    ...
    def _get_diff(self, input, keep_rgb=False):
        ...
        # input here is (batch_size, n_seg*samle_len, h, w)
        input_c = 3 if self.modality in ["RGB", "RGBDiff"] else 2
        input_view = input.view((-1, self.num_segments, self.new_length + 1, input_c,) + input.size()[2:])    # shape here is (batch_size, n_seg, new_length, channel, h, w)
        if keep_rgb:
